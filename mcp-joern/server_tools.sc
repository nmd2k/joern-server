...
def _get_classes(class_full_name: String, visited: mutable.Set[String] = mutable.Set()): List[nodes.TypeDecl] = {
    ...
    */
    ...
    val classDecls = cpg.typeDecl
      .filter(td =>
        td.fullName == class_full_name ||
        td.fullName.startsWith(class_full_name + "::") ||
        td.fullName.startsWith(class_full_name + ":")
      )
      .l
    if (classDecls.isEmpty) return List()
    val classDecl = classDecls.head
...
def load_cpg(cpg_filepath: String) :Boolean = {
    ...
    Updates the script-level `cpg` variable used by all custom tools.
    NOTE: to also update the Joern console's active project (for cpg.file.name.l etc.),
    call importCpg(cpg_filepath) directly in your query session before running traversals.
    ...
    try{
      if (cpg == null) {
        ...
        if (cur_cpg_filepath != cpg_filepath) {
          ...
          println(s"Closing old CPG at ${cur_cpg_filepath}")
          try {
            cpg.close()
          } catch {
            case _: Throwable => ()
          ...
          System.gc()
          cpg = CpgLoader.load(cpg_filepath)
          println(s"Loaded CPG at ${cpg_filepath}")
          return true
        }else{
...
def get_method_callees(method_full_name: String): List[String] = {
  ...
  */
  cpg.method.fullNameExact(method_full_name).headOption
    .map(_.callee.distinct.map(m => get_method_info_by_id(m.id)).l)
...
def get_method_callers(method_full_name: String): List[String] = {
  ...
  */
  cpg.method.fullNameExact(method_full_name).headOption
    .map(_.caller.distinct.map(m => get_method_info_by_id(m.id)).l)
    .getOrElse(List.empty[String])
}
...
def get_class_full_name_by_id(id: String): String = {
  ...
  */
  cpg.typeDecl.id(convertToLong(id)).headOption.map(_.fullName).getOrElse("")
}
...
def get_method_code_by_method_full_name(method_full_name: String): String = {
  ...
  */
  cpg.method.fullNameExact(method_full_name).headOption.map(_.sourceCode).getOrElse("")
}
...
def get_method_code_by_id(id: String): String = {
  ...
  @return: The code of the method
  */
  cpg.method.id(convertToLong(id)).headOption.map(_.sourceCode).getOrElse("")
}
...
def get_method_full_name_by_id(id: String): String = {
  ...
  */
  cpg.method.id(convertToLong(id)).headOption.map(_.fullName).getOrElse("")
}
...
def get_call_code_by_id(id: String): String = {
  ...
  */
  cpg.call.id(convertToLong(id)).headOption.map(_.code).getOrElse("")
}
...
def get_method_code_by_class_full_name_and_method_name(class_full_name: String, method_name: String): List[String] = {
  ...
  */
  cpg.typeDecl
    .filter(_.fullName == class_full_name)
    .flatMap(_.method.filter(_.name == method_name))
    .map(m => (s"methodFullName=${m.fullName} methodId=${m.id}L"))
    .l
}
...
def get_derived_classes_by_class_full_name(class_full_name: String): List[String] = {
  ...
  */
  cpg.typeDecl
    .filter(_.fullName == class_full_name)
    .flatMap(_.derivedTypeDeclTransitive)
    .map(c => get_class_info_by_id(c.id))
    .l
}
...
def get_parent_classes_by_class_full_name(class_full_name: String): List[String] = {
  ...
  */
  cpg.typeDecl
    ...
    .flatMap(parentFn => cpg.typeDecl.filter(_.fullName == parentFn))
    .map(c => get_class_info_by_id(c.id))
    .distinct
    .l
}
...
def get_method_by_call_id(id: String): String = {
  ...
  */
  cpg.call.id(convertToLong(id)).headOption
    .map(c => get_method_info_by_id(c.method.id))
    .getOrElse("")
}
...
def get_referenced_method_full_name_by_call_id(id: String): String = {
  ...
  @return: The method info of the call
  */
  cpg.call.id(convertToLong(id)).headOption.map(_.methodFullName).getOrElse("")
}
...
def get_calls_in_method_by_method_full_name(method_full_name: String): List[String] = {
   ...
   */
  cpg.method.fullNameExact(method_full_name).headOption
    .map(_.ast.isCall.map(c => get_call_info_by_id(c.id)).l)
    .getOrElse(List.empty[String])
}
