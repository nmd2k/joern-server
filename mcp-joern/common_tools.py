...
def extract_list(input_str):    
    ...
    # Strip trailing whitespace first so $ anchors correctly on the closing ')'.
    # Use greedy .* so nested parentheses in method full-names (e.g. "foo():void(int)")
    # don't cause the match to stop early.
    stripped = input_str.rstrip()
    list_pattern = r'List\((.*)\)$'
    list_match = re.search(list_pattern, stripped, re.DOTALL)
    if not list_match:
        return []
...
def extract_value(input_str: str) -> str:
    ...
    * If contains single quotes, uses extract_quoted_string
    ...
    elif 'Boolean =' in input_str:
        m = re.search(r'Boolean\s*=\s*(true|false)', input_str)
        if m:
            return m.group(1)
        return input_str
    else:
